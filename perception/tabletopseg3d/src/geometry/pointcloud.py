...
def project_mask_to_points(
    ...
    """Back-project mask pixels into 3D camera coordinates."""
...
def filter_points_by_depth_band(points_xyz: np.ndarray, z_mad_scale: float = 2.5) -> np.ndarray:
    """Reject obvious depth outliers while preserving the main object cluster."""
...
def plane_basis_from_normal(
    ...
    """Build a stable orthonormal basis whose Z axis follows the tabletop."""
...
def tabletop_aligned_obb(
    ...
    """Fit an oriented 3D box whose vertical axis is locked to the tabletop."""
    ...
    if len(points_xyz) >= 3:
        ...
        if covariance.shape == (2, 2) and np.all(np.isfinite(covariance)):
            # PCA is only applied in the tabletop plane, so the resulting box
            # keeps a physically meaningful "up" direction from the table.
